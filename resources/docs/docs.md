...
The `home-routes` handler in the `clojure-web-initializer.routes.home` namespace
...
The `home-page` function will in turn call the `clojure-web-initializer.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `clojure-web-initializer.handler` namespace:
...
Request middleware functions are located under the `clojure-web-initializer.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `clojure-web-initializer.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `clojure-web-initializer.db.core` in some other namespace.
